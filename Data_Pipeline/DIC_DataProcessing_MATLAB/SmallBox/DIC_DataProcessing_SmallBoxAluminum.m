...
% filepath for 1000N:
%file_path = "C:\Users\maria\Downloads\DIC_SmallBox_1000N.xlsx";

% filepath for 4000N:
file_path = "C:\Users\maria\Downloads\DIC_SmallBox_4000N.xlsx";
...
% % Drop rows where exx is NaN or 0
% data_filtered = data(~isnan(data.exx) & data.exx ~= 0, :);
% 
% % Create regular grid
% xi = linspace(min(data_filtered.X), max(data_filtered.X), 300);
...
% [X_grid, Y_grid] = meshgrid(xi, yi);
% 
% % Interpolate exx data onto the grid
% Z = griddata(data_filtered.X, data_filtered.Y, data_filtered.exx, X_grid, Y_grid, 'linear');
% 
% % Convert Z matrix into table format for saving
% Z_table = array2table(Z, 'VariableNames', string(xi));
% Z_table.Properties.RowNames = string(yi);
% 
% % Save interpolated grid as CSV
% writetable(Z_table, "C:\Users\maria\Documents\DIC\Z_df_exx.csv", 'WriteRowNames', false);
% 
% % Convert to long-format data
% [Y_long, X_long] = meshgrid(yi, xi);
% Z_long_exx = table(Y_long(:), X_long(:), Z(:), 'VariableNames', {'Y', 'X', 'exx'});
% 
% % Save long-format strain data as CSV
% writetable(Z_long_exx, "C:\Users\maria\Documents\DIC\Z_V1_exx.csv");